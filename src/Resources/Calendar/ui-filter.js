...
function initializeFilterContainer(container) {
  ...
  // ✨ Enhanced Search Wrapper (keeping original functionality)
  ...
  // ✅ Original search functionality with selectedValue
  searchInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      const finalFiltered = filtered.filter(ev => {
        ...
        if (selectedValue === "routing") return ev.id.startsWith("routing-");
        if (selectedValue === "production") return !ev.id.startsWith("routing-") && !ev.id.startsWith("comp-");
        if (selectedValue === "component") return ev.id.startsWith("comp-");
        return true;
      });
